...
from .utils.lstm import LSTMForSequenceOrdering


class OrderingBaselineLSTMModel(Model):
    """
    ...
    def __init__(
        ...
        if not onnx:
            self.model = (
                LSTMForSequenceOrdering.from_pretrained(model_name).eval().to(self.device)
            )
            if quantization:
